...
def embed_text(w):
    ...
    if w.get("subtitle"):
        parts.append("subtitle: " + w["subtitle"])
    if w.get("content_type"):
        parts.append("type: " + w["content_type"].lower())
...
def main():
    conn = db.get_conn()
    rows = conn.execute("""
        SELECT metadata_id, title, subtitle, authors, subjects, series, genres, content_type
